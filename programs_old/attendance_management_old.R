...
source(here("programs_old", "common_old.R"), encoding = "utf-8")
# ------ main ------
...
df_output_by_name <- map(name_list[[1]], function(target_name) {
  ...
  temp_max <- temp %>%
    ...
    distinct(clockout, .keep_all = T)
  temp_min <- temp %>%
    filter(time == min(time)) %>%
    select(name, ymd, clockin = time) %>%
    distinct(clockin, .keep_all = T)
  output_df <- full_join(temp_min, temp_max, by = c("name", "ymd")) %>%
    right_join(df_calender, by = "ymd") %>%
    arrange(ymd) %>%
    ungroup() %>%
  ...
  write.table(output_df, str_c(output_path, "/", output_file_name, ".txt"),
    sep = "\t", na = "",
