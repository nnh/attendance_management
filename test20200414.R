library(stringr)
library(dplyr)
input_path <- "/Volumes/Datacenter/IT/SystemAssistant/月例・随時作業関連/入退室ログ/ログデータ/202003/"
filelist <- list.files(input_path)[1:6]
...
b <- rbind(a[[1]], a[[2]], a[[3]], a[[4]], a[[5]], a[[6]]) %>% filter(!is.na(名前)) %>% select(c("日時", "状況", "名前"))
# カード作成, 削除情報は不要
